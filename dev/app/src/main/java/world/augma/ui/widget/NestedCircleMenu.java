...
public class NestedCircleMenu extends View {
    ...
    public NestedCircleMenu(Context context) {
        super(context);
        initView(context, null);
    }

    public NestedCircleMenu(Context context, AttributeSet attrs) {
        super(context, attrs);
        initView(context, attrs);
    }

    public NestedCircleMenu(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        initView(context, attrs);
    }

    @TargetApi(21)
    public NestedCircleMenu(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
    ...
    private void initView(Context context, AttributeSet attrs) {
        ...
        @ColorRes int colorRes = android.R.color.holo_green_dark;

        int smallRadius = 40;
        int bigRadius = 80;
        @DrawableRes int menuDrawable = R.drawable.ic_close_black_24dp;

        if (attrs != null) {
            TypedArray typedArray = getContext().getTheme().obtainStyledAttributes(
                    attrs,
                    R.styleable.NestedCircleMenu,
                    0, 0);
            try {
                if (typedArray.getResourceId(R.styleable.NestedCircleMenu_leftIcon, 0) != 0) {
                    mLeftDrawable = getDrawable(typedArray.getResourceId(R.styleable.NestedCircleMenu_leftIcon, 0));
                    updateDrawableSize(mLeftDrawable.getIntrinsicWidth(), mLeftDrawable.getIntrinsicHeight());
                }

                if (typedArray.getResourceId(R.styleable.NestedCircleMenu_rightIcon, 0) != 0) {
                    mRightDrawable = getDrawable(typedArray.getResourceId(R.styleable.NestedCircleMenu_rightIcon, 0));
                    updateDrawableSize(mLeftDrawable.getIntrinsicWidth(), mLeftDrawable.getIntrinsicHeight());
                }
                if (typedArray.getResourceId(R.styleable.NestedCircleMenu_topIcon, 0) != 0) {
                    mTopDrawable = getDrawable(typedArray.getResourceId(R.styleable.NestedCircleMenu_topIcon, 0));
                    updateDrawableSize(mLeftDrawable.getIntrinsicWidth(), mLeftDrawable.getIntrinsicHeight());
                }
                if (typedArray.getResourceId(R.styleable.NestedCircleMenu_bottomIcon, 0) != 0) {
                    mBottomDrawable = getDrawable(typedArray.getResourceId(R.styleable.NestedCircleMenu_bottomIcon, 0));
                    updateDrawableSize(mLeftDrawable.getIntrinsicWidth(), mLeftDrawable.getIntrinsicHeight());
                }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        int desiredWidth;
        int desiredHeight;
        desiredWidth = getContext().getResources().getDimensionPixelSize(R.dimen.widget_circle_menu_shrink_radius);
        desiredHeight = getContext().getResources().getDimensionPixelSize(R.dimen.widget_circle_menu_shrink_radius);
